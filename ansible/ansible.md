...
Файл починається з комбінації "---"
Весь файл це тест.
До кожного значення в форматі ключ та значення або перелік (перелік починається з символу "-").
Головне зберігати кількість пробілів на одному рівні. Для вкладених значень збільшується кількість пробілів. Сама кількість пробілів не має значення, має значення, щоб на одному рівні їх кількість була однаковою.
```
---
 - fruits:
   - apple
...
Тому що, модуль ping немає параметрів то в нього нічого не задано, а саме "ping:". Фактично назва таска, що задається в значенні name, використовується для інформування користувача та не є обов'язковою, тобто name: "ping" можна було упустити. Ці значення які задані в name, буде видно у виводі команди яка виконує плейбук.
...
Він встановлює Apache різними засобами в залежно від ОС, крім того, відбувається копіювання файлів і в разі якщо воно відбулося дається команда на перезапуск сервіса Apache.
